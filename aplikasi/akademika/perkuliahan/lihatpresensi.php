...
function Daftar()
{
    ...
    if ($_SESSION['Level'] == 'DOSEN') {
        ...
        echo '</div><div class="col-md-6">';
        $th =
            ' <th width="60" rowspan="2" align="center">VALIDASI</th> <th width="200" rowspan="2" align="center">keterangan</th>';
        ...
        while ($wr = $koneksi_db->sql_fetchassoc($result)) {
            ...
            $wm = $koneksi_db->sql_fetchassoc(
                $koneksi_db->sql_query(
                    "SELECT * FROM m_dosen where `idd` = '$idd' limit 1 "
                )
            );
            echo '<tr bgcolor="#f2f2f2">
                ...
                '</td>
				<td  align=left>' .
                viewAplikasi('09', '' . $wm['jabatan_akademik'] . '') .
                '</td>
				<td  align=center>' .
                hitungpresensidosen($prodi, $id, $kelas, $tahun_id, 'H', $idd) .
                '</td>
				<td  align=center>' .
                hitungpresensidosen($prodi, $id, $kelas, $tahun_id, 'S', $idd) .
                '</td>
				<td  align=center>' .
                hitungpresensidosen($prodi, $id, $kelas, $tahun_id, 'I', $idd) .
                '</td>
				<td  align=center>' .
                hitungpresensidosen($prodi, $id, $kelas, $tahun_id, 'A', $idd) .
                '</td>
    ...
    if ($jumlah > 0) {
        ...
        while ($w = $koneksi_db->sql_fetchassoc($pilih)) {
            $s = $koneksi_db->sql_fetchassoc(
				...
				kode_prodi='$prodi' and tahun_id='$tahun_id' and kelas='$kelas' and id='$id' and tanggal='" .
                        $w['tanggal'] .
                        "' and 
            ...
            echo '<tr >
                ...
                '</td>';
            if ($_SESSION['Level'] != 'DOSEN') {
                echo '<td align="left">	' . $w['verifikasi_bap'] . '</td>';
                echo '<td align="left">	' . $w['keterangan'] . '</td>';
            }
            echo '<td><a href="index.php?m=' .
                $_GET['m'] .
                '&op=edit&iddm=' .
                $w['iddm'] .
                '" >UBAH BAP</a> </td>
...
function edit()
{
    ...
    $s = $koneksi_db->sql_fetchassoc(
        $koneksi_db->sql_query(
            "select  bap  from t_dosen_presensi where iddm=$iddm"
        )
    );
    ...
    echo '<textarea name="bap" cols="45" rows="3"  class="required" id="bap" style="width:100%">' .
        $s[bap] .
        '</textarea>
	...
	<input type="button" class="btn" value="Kembali" onClick="window.location = \'index.php?m=' .
        $_GET['m'] .
        '\'"/>
				</th>	</tr></thead></form>';
}
...
function SIMPAN()
{
    ...
    $update = $koneksi_db->sql_query(
        "UPDATE `t_dosen_presensi` SET `bap` = '" .
            $_POST['bap'] .
            "'  WHERE `iddm` = '$iddm'"
    );
