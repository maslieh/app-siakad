...
global $koneksi_db;
		$iddm = $_GET['iddm'] ;
		$wp = $koneksi_db->sql_fetchassoc($koneksi_db->sql_query( "SELECT * FROM t_dosen_presensi where iddm='$iddm' limit 1 " ));
		$tahun = $wp['tahun_id'];
		$idl = $wp['id'];
		$mhs =	$koneksi_db->sql_query("delete from `t_mahasiswa_presensi`   WHERE tahun_id = '$tahun' and id='$idl'");
		$dose=	$koneksi_db->sql_query("delete from `t_dosen_presensi`   WHERE `iddm` = '$iddm'");
