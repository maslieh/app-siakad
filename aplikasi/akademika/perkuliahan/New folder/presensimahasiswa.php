...
function Daftar()
{
    ...
    if ($koneksi_db->sql_numrows($result) != 0) {
        while ($wr = $koneksi_db->sql_fetchassoc($result)) {
            ...
            echo '<a class="btn" href="" onclick="bukajendela(\'cetak.php?m=presensimahasiswa&idm=' .
                $id .
                '\'); return false">
    ...
    echo '<a class="btn" href="" onclick="bukajendela(\'cetak.php?m=cetakabsen\'); return false">
					<i class="fa fa-print"> </i> PRINT</a><br/>';
    echo $obj->total;
    echo '<br/>';
    echo $obj->anchors;
}
...
function GoInput()
{
 ...
 <form action="" method="post"  class="cmxform" id="form_input" style="width:100%">
		...
		';
    $tgl_absen = $koneksi_db->sql_fetchassoc(
        $koneksi_db->sql_query(
            "SELECT * FROM t_dosen_presensi where `kode_prodi` = '$prodi' AND `tahun_id` = '$tahun_id' AND id='$kode_mk' AND kelas='$kelas'  limit 1 "
        )
    );
    $tgl1 = date('Y-m-d');
    if ($_SESSION['Level'] == 'DOSEN') {
        ...
        //	}else {
        echo '<select name="tanggal"  required >' .
            tgl_ngajar($prodi, $tahun_id, $user, $kode_mk, $kelas, $_GET['m']) .
            '</select>';
        //	}
    } elseif ($_SESSION['Level'] == 'PA') {
        echo '<select name="tanggal"  required >' .
        ...
        echo '<select name="tanggal"  required >' .
            tgl_ngajar_admin($prodi, $tahun_id, $kode_mk, $kelas, $_GET['m']) .
            '</select>';
    }
    echo '</td></tr>
    ...
    if ($jumlah > 0) {
        ...
        while ($wr = $koneksi_db->sql_fetchassoc($pilih)) {
            ...
            echo '<tr bgcolor="#f2f2f2">
                ...
                '</td>
				<td  align=left>' .
                NamaTahun($wm['batas_studi'], $wm['kode_prodi']) .
                '</td>
					<td  align=center><input type="radio" ' .
                $ck .
                ' name="jenis_presensi[' .
                $id .
                ']" checked="checked" value="H"></td>
					<td  align=center><input type="radio" ' .
                $ck .
                ' name="jenis_presensi[' .
                $id .
    ...
    if (is_array($_POST['idm'])) {
        foreach ($_POST['idm'] as $key => $val) {
            ...
            $wi = $koneksi_db->sql_fetchassoc(
                $koneksi_db->sql_query(
                    "SELECT * FROM t_mahasiswa_krs where idkrs='$key' limit 1 "
                )
            );
            ...
            $ada = $koneksi_db->sql_numrows(
                $koneksi_db->sql_query(
                    "SELECT * FROM t_mahasiswa_presensi $strwhr"
                )
            );

            if ($ada < 1) {
                $s =
                    "insert into t_mahasiswa_presensi SET 
					kode_pt='" .
                    $wi['kode_pt'] .
                    "',
					kode_fak='" .
    ...
    if ($jumlah > 0) {
        ...
        while ($w = $koneksi_db->sql_fetchassoc($pilih)) {
            ...
            while ($r = $koneksi_db->sql_fetchrow($query)) {
                ...
                echo '<td  align=center><input type="radio" ' .
                    $ck .
                    ' name="jenis_presensi[' .
                    $id .
                    ']" value="' .
    ...
    if (is_array($_POST['jenis_presensi'])) {
        foreach ($_POST['jenis_presensi'] as $key => $val) {
            $update = $koneksi_db->sql_query(
                "UPDATE `t_mahasiswa_presensi` SET `jenis_presensi` = '$val' WHERE `idpm` = '$key'"
            );

            $wi = $koneksi_db->sql_fetchassoc(
                $koneksi_db->sql_query(
                    "SELECT * FROM t_mahasiswa_presensi where idpm='$key' limit 1 "
                )
            );
            $presensi_dosen = total_presensi_dosen(
                $wi['kode_prodi'],
